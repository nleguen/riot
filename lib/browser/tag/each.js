...
function _each(dom, parent, expr) {
  ...
  parent.one('premount', function () {
    ...
    each(items, function(item, i) {
      var oldPos = oldItems.indexOf(item, i),
        pos = ~oldPos ? oldPos : i,
      ...
      // reorder the tag if it's not in its previous position
      if (pos !== i) {
        // move the old tag into the right position
        root.insertBefore(tag.root, tags[i].root)
        // update the position position attribute if it exists
        if (expr.pos)
          tag[expr.pos] = i
        // move the old tag instance
        tags.splice(i, 0, tags.splice(pos, 1)[0])
        // move the old item into the right position
